...
impl TryFrom<PathBuf> for CertificateKeyPair {
	...
	fn try_from(path: PathBuf) -> Result<Self, Self::Error> {
		...
		let ca_key_pem =
			fs::read_to_string(&ca_key_path).map_err(|e| Error::new(ca_key_path.clone(), ErrorKind::IO(e)))?;
		let ca_key =
			KeyPair::from_pem(ca_key_pem.as_str()).map_err(|e| Error::new(ca_key_path, ErrorKind::Certificate(e)))?;

		let ca_cert_path = path.with_extension("pem");
		let ca_cert_pem =
			fs::read_to_string(&ca_cert_path).map_err(|e| Error::new(ca_cert_path.clone(), ErrorKind::IO(e)))?;
		...
		let ca_cert =
			ca_cert_params.self_signed(&ca_key).map_err(|e| Error::new(ca_cert_path, ErrorKind::Certificate(e)))?;
